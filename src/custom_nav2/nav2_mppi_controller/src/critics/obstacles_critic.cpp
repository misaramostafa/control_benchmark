...
void ObstaclesCritic::initialize()
{
  ...
  if (possibly_inscribed_cost_ < 1.0f) {
    RCLCPP_WARN(
      ...
      " for full instructions. possibly_inscribed_cost_ will be refreshed each control cycle.");
  }
  ...
  std::shared_ptr<nav2_costmap_2d::Costmap2DROS> costmap)
{
  ...
  // Only use cached value if it is valid (>= 1.0) AND the footprint hasn't changed.
  // If circumscribed_cost_ < 1.0, the inflation layer may not have been ready yet
  // when the cache was populated, so we must re-evaluate.
  if (static_cast<float>(circum_radius) == circumscribed_radius_ &&
    circumscribed_cost_ >= 1.0f)
  {
    // early return only when cached value is valid
  ...
  // Only cache radius when the result is valid so we retry next cycle if not ready yet
  if (result >= 1.0) {
...
void ObstaclesCritic::score(CriticData & data)
{
  ...
  // Refresh possibly_inscribed_cost_ every cycle: the inflation layer's inscribed
  // radius may not be finalized at configure time (it is set during activate/first
  // footprint update). The early-return in findCircumscribedCost makes this cheap
  // once the value is valid.
  possibly_inscribed_cost_ = findCircumscribedCost(costmap_ros_);
  // NOTE: if possibly_inscribed_cost_ < 1.0 we still run collision checking below;
  // we only skip the repulsion / distanceToObstacle term which needs inflation math.
